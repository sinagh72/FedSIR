...
from dataset.noise import add_noise
...
from algorithm.strategy.fedavg import run_fedavg
from algorithm.strategy.fedprox import run_fedprox
from algorithm.strategy.pruning import run_pruning
from algorithm.strategy.fedsir import run_fedsir
...
def build_client_loaders(cfg, noisy_dataset, y_noisy, y_clean, dict_users):
    ...
    for cid in range(cfg.num_users):
        ...
        base_train.transform = build_transforms(cfg.dataset, split="train")
        ...
        base_plain.transform = build_transforms(cfg.dataset, split="test")
        ds_plain = NoisyLabelDataset(base_plain, y_noisy=y_noisy, y_clean=y_clean)
        ds_client_plain = Subset(ds_plain, sample_idx)
        train_loaders_plain[cid] = build_loader(ds_client_plain, batch_size=cfg.batch_size, shuffle=False, num_workers=4, seed=cfg.seed, drop_last=False)
...
def run_fl(cfg, dataset_train, dataset_test, dict_users, num_classes, forced_gamma_s=None):
    ...
    ablation_tag = getattr(cfg, "ablation_name", "default")
    save_dir = (
        f"{cfg.save_dir}/{cfg.dataset}_{cfg.noise_type}{distribution}/"
        f"{gamma_s.tolist().count(0)} clean/{cfg.noise_p}/{cfg.fl_strategy}/{ablation_tag}"
    )
    ...
    cfg_identification = TrainConfig(epochs=cfg.identification_epochs,
                            lr=cfg.identification_lr,
                            weight_decay=cfg.identification_weight_decay,
                            optimizer="Adam")
    ...
    if cfg.fl_strategy == "fedavg":
        ...
        run_fedprox(cfg=cfg, gamma_s=gamma_s, num_classes=num_classes, base_state=base_state, train_loaders_train=train_loaders_train, test_loader=test_loader,
            cfg_train=cfg_train, save_dir=save_dir, device=device)
    
         
    elif cfg.fl_strategy == "pruning": 
        run_pruning(cfg=cfg, gamma_s=gamma_s, num_classes=num_classes, base_state=base_state, train_loaders_train=train_loaders_train,
                            test_loader=test_loader, cfg_train=cfg_train, cfg_identification=cfg_identification, save_dir=save_dir, device=device)
    
    elif cfg.fl_strategy == "fedsir":
        run_fedsir(
            cfg=cfg,
            gamma_s=gamma_s,
            num_classes=num_classes,
